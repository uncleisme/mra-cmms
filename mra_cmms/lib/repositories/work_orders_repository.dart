...
class WorkOrdersRepository {
  ...
  WorkOrdersRepository({SupabaseClient? client, Box? box})
    : _client = client ?? Supabase.instance.client,
      _box = box ?? Hive.box('work_orders');
  ...
  Future<(bool ok, String? error)> createWorkOrder({
    ...
    try {
      ...
      // Restore: Notification logic for new work order
      try {
        final notificationsRepo = NotificationsRepository();
        final recipients = <String>{};
        if (requestedBy != null && requestedBy.isNotEmpty)
          recipients.add(requestedBy);
        if (assignedTo != null && assignedTo.isNotEmpty)
          recipients.add(assignedTo);
        if (recipients.isNotEmpty) {
          await notificationsRepo.create(
            userId: requestedBy ?? assignedTo ?? '',
            module: 'Work Orders',
            action: 'created',
            entityId: id,
            message: 'Work order "$workOrderId" has been created',
            recipients: recipients.toList(),
          );
        }
      } catch (e) {
        // Log but do not fail work order creation if notification fails
      }
  ...
  /// Returns work orders assigned to the current user that are due today (by appointmentDate or nextScheduledDate).
  Future<List<WorkOrder>> getTodaysAssigned() async {
    ...
    DateTime? effectiveDate(WorkOrder wo) {
      ...
      final due = wo.appointmentDate?.toLocal();
      final completed =
    ...
    debugPrint(
      'getTodaysAssigned: fetched ${parsed.length} work orders: ${parsed.map((wo) => 'id: ${wo.id}, appointment: ${wo.appointmentDate}, next: ${wo.nextScheduledDate}, status: ${wo.status}').join(', ')}',
    );
    ...
    debugPrint(
      'getTodaysAssigned: filtered today: ${filtered.map((wo) => wo.id).join(', ')}',
    );
    return filtered;
  }
  ...
  Future<List<WorkOrder>> getAssignedToMe() async {
    ...
    final items = (res as List)
        .map((e) => Map<String, dynamic>.from(e as Map))
        .toList();
    return await compute(_parseWorkOrders, items);
  }
  ...
  // getKpis removed - no longer used
